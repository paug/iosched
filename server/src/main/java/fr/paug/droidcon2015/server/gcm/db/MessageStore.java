...
import fr.paug.droidcon2015.server.gcm.db.models.MulticastMessage;
...
public final class MessageStore {
    ...
    public static Long createMulticast(List<String> devices,
        ...
        OfyService.ofy().save().entity(msg).now();
        Long id = msg.getId();
    ...
    public static MulticastMessage getMulticast(Long id) {
        return OfyService.ofy().load().type(MulticastMessage.class).id(id).now();
    }
    ...
    public static void updateMulticast(Long id, List<String> devices) {
        MulticastMessage msg = OfyService.ofy().load().type(MulticastMessage.class).id(id).now();
        if (msg == null) {
            LOG.severe("No entity for multicast ID: " + id);
            return;
        }
        msg.setDestinations(devices);
        OfyService.ofy().save().entity(msg);
    }
    ...
    public static void deleteMulticast(Long id) {
        OfyService.ofy().delete().type(MulticastMessage.class).id(id);
    }
