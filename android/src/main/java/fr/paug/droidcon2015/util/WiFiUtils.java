...
package fr.paug.droidcon2015.util;
...
public class WiFiUtils {
    ...
    public static class WiFiDialog extends DialogFragment {
        private static final String ARG_WIFI_ENABLED
                = "fr.paug.droidcon2015.ARG_WIFI_ENABLED";
    ...
    public static boolean shouldOfferToSetupWifi(final Context context, boolean actively) {
        ...
        if (now < Config.WIFI_SETUP_OFFER_START) {
            LogUtils.LOGW(TAG, "Too early to offer wifi");
            return false;
        }
        if (now > Config.CONFERENCE_END_MILLIS) {
            LogUtils.LOGW(TAG, "Too late to offer wifi");
            return false;
        }
        if (!WiFiUtils.isWiFiEnabled(context)) {
            LogUtils.LOGW(TAG, "Wifi isn't enabled");
            return false;
        }
        if (!SettingsUtils.isAttendeeAtVenue(context)) {
            LogUtils.LOGW(TAG, "Attendee isn't onsite so wifi wouldn't matter");
            return false;
        }
        if (WiFiUtils.isWiFiApConfigured(context)) {
            LogUtils.LOGW(TAG, "Attendee is already setup for wifi.");
            return false;
        }
        if (actively && SettingsUtils.hasDeclinedWifiSetup(context)) {
            LogUtils.LOGW(TAG, "Attendee opted out of wifi.");
            return false;
        }
        return true;
    }
